...
public class EnemyBloodwaveStrategy: SkillStrategy
{
	public override bool Call(MovementController movementController, Team team)
	{
		...
		for (int i = 0; i < _storedSkillData.numberOfProjectiles; i++)
		{
            ...
            {
                startingPosition = movementController.transform.position + 0f * (Quaternion.AngleAxis(i * (360f / _storedSkillData.numberOfProjectiles), Vector3.up) * movementController.GetFacingDirection()),
			...
			Destroy(p.gameObject, projectileData.Lifetime);
        }
